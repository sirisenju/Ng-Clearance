...
class StudentAffairsClearance extends StatefulWidget {
  const StudentAffairsClearance({super.key});

  @override
  State<StudentAffairsClearance> createState() => _StudentAffairsClearanceState();
}

class _StudentAffairsClearanceState extends State<StudentAffairsClearance> {
  ...
  Future uploadFiles(String userUid, File documentFile, String filename) async {
    ...
    await FirebaseFirestore.instance.collection('user_documents')
        .doc(userUid)
        .collection('documents').doc("student_affairs_clearance").set({
      'filename': filename,
      'url': downloadURL,
