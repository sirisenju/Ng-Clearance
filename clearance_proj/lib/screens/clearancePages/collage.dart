...
class CollageClearance extends StatefulWidget {
  const CollageClearance({super.key});

  @override
  State<CollageClearance> createState() => _CollageClearanceState();
}

class _CollageClearanceState extends State<CollageClearance> {
  ...
  Future uploadFiles(String userUid, File documentFile, String filename) async {
    ...
    await FirebaseFirestore.instance.collection('user_documents')
        .doc(userUid)
        .collection('documents').doc("collage_clearance").set({
      'filename': filename,
      'url': downloadURL,
          ...
          child: Column(
            ...
            children: [
              ...
              const Padding(
                ...
                child: Text("Welcome to your Collage Clearance Guide.", style: TextStyle(fontSize: 20, fontWeight: FontWeight.w500)
                ),
              ),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 2.0, horizontal: 8.0),
                child: Text(
                    "Pls make sure to submit the following documents listed below in a single pdf", style: TextStyle(fontSize: 18)
                ),
              ),
              ListView.builder(
                  shrinkWrap: true,
